...
export default function ClientsPage() {
  ...
  const reload = () => fetch("/api/clients").then((r) => r.json()).then(setClients).catch(() => {});
    ...
    <div className="p-7 space-y-6">
      ...
      {clients.length === 0 ? (
        <div className="bg-white rounded-2xl border-2 border-dashed border-zinc-200 flex flex-col items-center justify-center py-20">
          ...
          <p className="text-xs text-zinc-300 mt-1">Agrega tu primer cliente con el botón de arriba</p>
        </div>
      ) : (
          ...
          {clients.map((c) => (
            <div key={c.id} className="bg-white rounded-2xl p-5 shadow-sm border border-zinc-100 hover:shadow-md transition-shadow cursor-pointer">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-[#c9a566]/10 flex items-center justify-center shrink-0">
